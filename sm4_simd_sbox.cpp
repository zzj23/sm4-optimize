...
#define MM256_PACK0_EPI32(a, b, c, d)                  \
    _mm256_unpacklo_epi64(_mm256_unpacklo_epi32(a, b), \
                          _mm256_unpacklo_epi32(c, d)) //低位128bit交叉
...
void _SM4_do(uint8_t* in, uint8_t* out, SM4_Key sm4_key, int enc) {
    ...
    //加载数据
    Temp[0] = _mm256_loadu_si256((const __m256i*)in + 0);//00，01，02，03，10~13
    ...
    X[0] = _mm256_unpacklo_epi32(Temp[0], Temp[1]);//00，10，01，21，10，30，11，31
    X[0] = _mm256_unpacklo_epi32(Temp[2], Temp[3]);
    X[0] = MM256_PACK0_EPI32(Temp[0], Temp[1], Temp[2], Temp[3]);//low->high 00，20，40，60，20，30，50，70
    ...
    __m256i vindex =
        _mm256_setr_epi8(3, 2, 1, 0, 7, 6, 5, 4, 11, 10, 9, 8, 15, 14, 13, 12,
            3, 2, 1, 0, 7, 6, 5, 4, 11, 10, 9, 8, 15, 14, 13, 12);//反向排序并存入
    ...
    for (int i = 0; i < 32; i++) {
        ...
        Temp[0] = _mm256_xor_si256(_mm256_xor_si256(X[1], X[2]),
            _mm256_xor_si256(X[3], k));//tmp = x[1] ^ x[2] ^ x[3] ^ k;
        //查表
        Temp[1] = _mm256_and_si256(Temp[0], Mask);
        Temp[1] = _mm256_xor_si256(
            X[0], _mm256_i32gather_epi32((const int*)BOX0,
                _mm256_and_si256(Temp[0], Mask), 4));//tmp1 = x[0]^Table3[(tmp >> 0) & 0xFF]
        Temp[0] = _mm256_srli_epi32(Temp[0], 8);//tmp = tmp>>8
        Temp[1] = _mm256_xor_si256(
            Temp[1], _mm256_i32gather_epi32(
                (const int*)BOX1, _mm256_and_si256(Temp[0], Mask), 4));//tmp1 ^= Table2[(tmp >> 8) & 0xFF]
        Temp[0] = _mm256_srli_epi32(Temp[0], 8);//tmp = tmp>>8
        Temp[1] = _mm256_xor_si256(
            Temp[1], _mm256_i32gather_epi32(
                (const int*)BOX2, _mm256_and_si256(Temp[0], Mask), 4));//tmp1 ^= Table1[(tmp >> 16) & 0xFF];
        Temp[0] = _mm256_srli_epi32(Temp[0], 8);//tmp = tmp>>8
        Temp[1] = _mm256_xor_si256(
            Temp[1], _mm256_i32gather_epi32(
                (const int*)BOX3, _mm256_and_si256(Temp[0], Mask), 4));//tmp1 ^= Table1[(tmp >> 24) & 0xFF];
